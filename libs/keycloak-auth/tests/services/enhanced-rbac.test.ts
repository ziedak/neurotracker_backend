...
describe("EnhancedRBACService", () => {
  ...
  const MOCK_ACCESS_TOKEN = createMockJWT({
    ...
    resource_access: {
      "test-client": { roles: ["app_user"] },
    },
  });

  const MOCK_ADMIN_TOKEN = createMockJWT({
    realm_access: { roles: ["admin"] },
    resource_access: {
      "test-client": { roles: ["app_admin"] },
    },
  });
  ...
  describe("Permission Checking", () => {
    ...
    it("should deny access when no permissions match", async () => {
      ...
      expect(decision.reason).toContain("insufficient_permissions");
    });
    ...
    it("should handle permission check errors gracefully", async () => {
      ...
      expect(decision.allowed).toBe(false);
      expect(decision.reason).toBe("rbac_check_error");
  ...
  describe("Policy Synchronization", () => {
    it("should sync policies to Keycloak", async () => {
      ...
      expect(mockAuthzClient.createPolicy).toHaveBeenCalledTimes(4); // 2 roles + 2 inheritance policies
    });
...
describe("RBACHelpers", () => {
  describe("Hierarchy Creation", () => {
    ...
    it("should create enterprise hierarchy", () => {
      ...
      expect(hierarchy["department_manager"].inherits).toContain("team_lead");
...
describe("RBACHelpers", () => {
  describe("Hierarchy Creation", () => {
    ...
    it("should create enterprise hierarchy", () => {
      ...
      expect(hierarchy.global_admin.inherits).toContain("tenant_admin");
      expect(hierarchy.tenant_admin.inherits).toContain("department_manager");
      expect(hierarchy.department_manager.inherits).toContain("team_lead");
      expect(hierarchy.team_lead.inherits).toContain("employee");
      expect(hierarchy.employee.inherits).toContain("guest");
    });
