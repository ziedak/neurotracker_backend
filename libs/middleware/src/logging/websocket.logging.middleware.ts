 ...
 */
export interface WebSocketLoggingConfig extends WebSocketMiddlewareConfig {
 ...
 *
 * @template WebSocketLoggingConfig - WebSocket logging-specific configuration
 */
export class WebSocketLoggingMiddleware extends BaseWebSocketMiddleware<WebSocketLoggingConfig> {
  private connectionStartTimes = new Map<string, number>();
  private connectionMessageCounts = new Map<string, number>();

  constructor(
    metrics: IMetricsCollector,
    config: Partial<WebSocketLoggingConfig> = {}
  ) {
    // Create complete configuration with validated defaults
    const completeConfig: WebSocketLoggingConfig = {
   ...
   */
  static createDevelopmentConfig(): Partial<WebSocketLoggingConfig> {
    return {
   ...
   */
  static createProductionConfig(): Partial<WebSocketLoggingConfig> {
    return {
   ...
   */
  static createAuditConfig(): Partial<WebSocketLoggingConfig> {
    return {
   ...
   */
  static createMinimalConfig(): Partial<WebSocketLoggingConfig> {
    return {
   ...
   */
  static createPerformanceConfig(): Partial<WebSocketLoggingConfig> {
    return {
   ...
   */
  static createDebugConfig(): Partial<WebSocketLoggingConfig> {
    return {
 ...
 */
export function createWebSocketLoggingMiddleware(
  metrics: IMetricsCollector,
  config?: Partial<WebSocketLoggingConfig>
): WebSocketLoggingMiddleware {
  return new WebSocketLoggingMiddleware(metrics, config);
}
...
export const WEBSOCKET_LOGGING_PRESETS = {
  development: (): Partial<WebSocketLoggingConfig> =>
    WebSocketLoggingMiddleware.createDevelopmentConfig(),

  production: (): Partial<WebSocketLoggingConfig> =>
    WebSocketLoggingMiddleware.createProductionConfig(),

  audit: (): Partial<WebSocketLoggingConfig> =>
    WebSocketLoggingMiddleware.createAuditConfig(),

  minimal: (): Partial<WebSocketLoggingConfig> =>
    WebSocketLoggingMiddleware.createMinimalConfig(),

  performance: (): Partial<WebSocketLoggingConfig> =>
    WebSocketLoggingMiddleware.createPerformanceConfig(),

  debug: (): Partial<WebSocketLoggingConfig> =>
    WebSocketLoggingMiddleware.createDebugConfig(),
} as const;
