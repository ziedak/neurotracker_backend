...
export class RuleValidator implements Validator {
  ...
  async validate(context: MiddlewareContext): Promise<ValidationResult> {
    ...
    try {
      ...
      if (this.config.validateBody && ruleSet.body && context.request.body) {
        const bodyErrors = this.validateFields(context.request.body, ruleSet.body, 'body');
      ...
      if (this.config.validateQuery && ruleSet.query && context.request.query) {
        const queryErrors = this.validateFields(context.request.query, ruleSet.query, 'query');
      ...
      if (this.config.validateParams && ruleSet.params && context.request.params) {
        const paramsErrors = this.validateFields(context.request.params, ruleSet.params, 'params');
   ...
   */
  private validateFields(data: any, rules: ValidationRule[], context: string): ValidationError[] {
    const errors: ValidationError[] = [];
   ...
   */
  private getRuleSetForContext(context: MiddlewareContext): ValidationRuleSet | null {
