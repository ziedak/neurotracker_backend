...
import {
  ...
  type PrometheusMiddlewareConfig,
} from "./PrometheusMiddleware";
import {
  PrometheusWebSocketMiddleware,
  type PrometheusWebSocketMiddlewareConfig,
} from "./PrometheusWebSocketMiddleware";
...
export const PROMETHEUS_PRESETS = {
   ...
   */
  apiGateway(): Partial<PrometheusMiddlewareConfig> {
    return {
   ...
   */
  microservice(): Partial<PrometheusMiddlewareConfig> {
    return {
   ...
   */
  debug(): Partial<PrometheusMiddlewareConfig> {
    return {
...
export function createPrometheusMiddleware(
  ...
  return new PrometheusMiddleware(metrics, config);
}
 ...
 */
export function createPrometheusMiddlewareWithPreset(
  metrics: IMetricsCollector,
  preset: () => Partial<PrometheusMiddlewareConfig>,
  overrides: Partial<PrometheusMiddlewareConfig> = {}
): PrometheusMiddleware {
  const presetConfig = preset();
  const finalConfig = { ...presetConfig, ...overrides };

  return new PrometheusMiddleware(metrics, finalConfig);
}
...
export const PROMETHEUS_TESTING_UTILS = {
   ...
   */
  createMockMiddleware(config: Partial<PrometheusMiddlewareConfig> = {}) {
    const mockMetrics = {
    ...
    const middleware = new PrometheusMiddleware(mockMetrics, {
