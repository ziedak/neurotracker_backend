...
export class AuthMiddleware extends BaseMiddleware<AuthConfig> {
  ...
  async execute(context: MiddlewareContext, next: () => Promise<void>): Promise<void | any> {
    ...
    try {
      ...
      if (this.shouldBypassAuth(context)) {
        ...
        await this.recordTimer('auth_bypass_duration', performance.now() - startTime);
        return next();
      }
      ...
      if (authResult.rateLimitRemaining !== undefined) {
        context.set.headers['X-RateLimit-Remaining'] = authResult.rateLimitRemaining.toString();
      }
   ...
   * Check authorization (roles and permissions)
   */
  private async authorize(authResult: AuthResult, context: MiddlewareContext): Promise<{ authorized: boolean; error?: string }> {
    if (!authResult.user || authResult.user.anonymous) {
      return {
        authorized: this.config.allowAnonymous || false,
        error: this.config.allowAnonymous ? undefined : 'Anonymous access not allowed'
      };
    }
   ...
   */
  public static create(type: 'api-gateway' | 'ai-engine' | 'data-intelligence' | 'event-pipeline', overrides?: Partial<AuthConfig>): AuthMiddleware {
    const configs = {
      'api-gateway': {
        allowAnonymous: true,
