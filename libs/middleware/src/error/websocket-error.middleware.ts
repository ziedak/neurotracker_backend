...
export interface WebSocketErrorMiddlewareConfig
 ...
 */
export class WebSocketErrorMiddleware extends BaseWebSocketMiddleware<WebSocketErrorMiddlewareConfig> {
  constructor(
    metrics: IMetricsCollector,
    config: WebSocketErrorMiddlewareConfig
  ) {
    const mergedConfig = {
      ...config,
      ...{
    ...
    } as WebSocketErrorMiddlewareConfig;
  ...
  protected async execute(
    ...
    try {
      ...
      await this.handleWebSocketError(error as Error, context);
    }
  }
  ...
  private async handleWebSocketError(
    ...
    const errorResponse = await this.createWebSocketErrorResponse(
      error,
      context
    );
    ...
    if (this.config.logErrors) {
      await this.logWebSocketError(error, context);
    }
  }
   ...
   */
  public async createWebSocketErrorResponse(
    error: Error | WebSocketCustomError,
    context?: WebSocketContext
  ): Promise<WebSocketErrorResponse> {
    try {
   ...
   */
  public async handleAsyncWebSocketError(
    ...
    try {
      ...
      return this.createWebSocketErrorResponse(error as Error, context);
    }
  }
  ...
  ): (...args: T) => Promise<R | WebSocketErrorResponse> {
    return async (...args: T) => {
      try {
        return await handler(...args);
      } catch (error) {
        return this.createWebSocketErrorResponse(error as Error);
      }
    };
  }
   ...
   */
  private async logWebSocketError(
    error: Error | WebSocketCustomError,
    context: WebSocketContext
  ): Promise<void> {
   ...
   */
  static createDevelopmentConfig(): Partial<WebSocketErrorMiddlewareConfig> {
    return {
  ...
  static createProductionConfig(): Partial<WebSocketErrorMiddlewareConfig> {
    return {
      includeStackTrace: false,
      logErrors: true,
      errorResponseType: "error",
  ...
  static createMinimalConfig(): Partial<WebSocketErrorMiddlewareConfig> {
    return {
      includeStackTrace: false,
      logErrors: false,
      errorResponseType: "error",
  ...
  static createAuditConfig(): Partial<WebSocketErrorMiddlewareConfig> {
    return {
      includeStackTrace: true,
      logErrors: true,
      errorResponseType: "error",
 ...
 */
export function createWebSocketErrorMiddleware(
  metrics: IMetricsCollector,
  config?: Partial<WebSocketErrorMiddlewareConfig>
): WebSocketErrorMiddleware {
  const defaultConfig: WebSocketErrorMiddlewareConfig = {
  ...
  return new WebSocketErrorMiddleware(metrics, defaultConfig);
}
