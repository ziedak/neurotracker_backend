 ...
 */
export interface WebSocketCorsMiddlewareConfig
 ...
 */
export interface WebSocketCorsContext extends WebSocketContext {
  upgradeHeaders?: Record<string, string | string[] | undefined>;
  websocket?: any; // For error response sending
}
...
const DEFAULT_WS_CORS_OPTIONS = {
  ORIGIN: "*" as const,
  ALLOWED_PROTOCOLS: ["ws", "wss"] as const,
  ALLOWED_EXTENSIONS: ["permessage-deflate", "x-webkit-deflate-frame"] as const,
  CREDENTIALS: true,
...
/**
 * WebSocket CORS origin validation result
 */
...
/**
 * WebSocket upgrade headers interface
 */
interface WebSocketUpgradeHeaders {
  readonly origin?: string | undefined;
  readonly upgrade?: string | undefined;
  readonly connection?: string | undefined;
  readonly secWebSocketKey?: string | undefined;
  readonly secWebSocketVersion?: string | undefined;
  readonly secWebSocketProtocol?: string | undefined;
  readonly secWebSocketExtensions?: string | undefined;
}
 ...
 *
 * @template WebSocketCorsMiddlewareConfig - WebSocket CORS-specific configuration
 */
export class WebSocketCorsMiddleware extends BaseWebSocketMiddleware<WebSocketCorsMiddlewareConfig> {
  constructor(
    metrics: IMetricsCollector,
    config: Partial<WebSocketCorsMiddlewareConfig> = {}
  ) {
    // Create complete configuration with validated defaults
    const completeConfig: WebSocketCorsMiddlewareConfig = {
      ...
      validateUpgrade:
        config.validateUpgrade ?? DEFAULT_WS_CORS_OPTIONS.VALIDATE_UPGRADE,
      allowOriginless:
        config.allowOriginless ?? DEFAULT_WS_CORS_OPTIONS.ALLOW_ORIGINLESS,
    };
   ...
   */
  private async validateConnectionUpgrade(
    context: WebSocketContext
  ): Promise<void> {
    ...
    if (!originValidation.allowed) {
      throw new Error(
        `WebSocket CORS origin validation failed: ${originValidation.reason}`
      );
    }
   ...
   */
  private async validateMessageOrigin(
    context: WebSocketContext
  ): Promise<void> {
    const connectionId = this.getConnectionId(context);
   ...
   */
  private validateUpgradeHeaders(headers: WebSocketUpgradeHeaders): void {
    ...
    if (headers.secWebSocketVersion !== "13") {
      throw new Error(
        `Unsupported WebSocket version: ${headers.secWebSocketVersion}`
      );
    }
  }
   ...
   */
  private validateProtocols(protocolHeader: string): void {
    ...
    const hasValidProtocol = requestedProtocols.some((protocol) =>
      allowedProtocols.includes(protocol)
    );

    if (!hasValidProtocol) {
      throw new Error(
        `No allowed WebSocket protocols found. Requested: ${requestedProtocols.join(
          ", "
        )}`
      );
    }
   ...
   */
  private validateExtensions(extensionsHeader: string): void {
    ...
    const invalidExtensions = requestedExtensions.filter(
      (ext) => ext && !allowedExtensions.includes(ext)
    );

    if (invalidExtensions.length > 0) {
      throw new Error(
        `Unsupported WebSocket extensions: ${invalidExtensions.join(", ")}`
      );
    }
  }
   ...
   */
  private async handleWebSocketCorsError(
    error: unknown,
    context: WebSocketContext
  ): Promise<void> {
    const connectionId = this.getConnectionId(context);
    const errorMessage =
      error instanceof Error ? error.message : "Unknown WebSocket CORS error";
   ...
   */
  private async recordWebSocketCorsMetrics(
    action: string,
   ...
   */
  static createDevelopmentConfig(): Partial<WebSocketCorsMiddlewareConfig> {
    return {
  ...
  ): Partial<WebSocketCorsMiddlewareConfig> {
    return {
 ...
 */
export function createWebSocketCorsMiddleware(
  metrics: IMetricsCollector,
  config?: Partial<WebSocketCorsMiddlewareConfig>
): WebSocketCorsMiddleware {
  return new WebSocketCorsMiddleware(metrics, config);
}
...
export const WEBSOCKET_CORS_PRESETS = {
  development: (): Partial<WebSocketCorsMiddlewareConfig> =>
    WebSocketCorsMiddleware.createDevelopmentConfig(),

  production: (
    origins: readonly string[]
  ): Partial<WebSocketCorsMiddlewareConfig> =>
    WebSocketCorsMiddleware.createProductionConfig(origins),

  realtime: (
    origins: readonly string[]
  ): Partial<WebSocketCorsMiddlewareConfig> =>
    WebSocketCorsMiddleware.createRealtimeConfig(origins),

  strict: (
    origins: readonly string[]
  ): Partial<WebSocketCorsMiddlewareConfig> =>
    WebSocketCorsMiddleware.createStrictConfig(origins),
