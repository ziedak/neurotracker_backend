 ...
 */
export abstract class BaseMiddleware<TConfig extends MiddlewareOptions = MiddlewareOptions> {
   ...
   */
  protected async recordMetric(name: string, value: number = 1, tags?: Record<string, string>): Promise<void> {
    if (this.metrics) {
      try {
        await this.metrics.recordCounter(name, value, tags);
      } catch (error) {
        this.logger.warn('Failed to record metric', { name, error: (error as Error).message });
      }
    }
  }
   ...
   */
  protected async recordTimer(name: string, duration: number, tags?: Record<string, string>): Promise<void> {
    if (this.metrics) {
      try {
        await this.metrics.recordTimer(name, duration, tags);
      } catch (error) {
        this.logger.warn('Failed to record timer', { name, error: (error as Error).message });
      }
    }
  }
   ...
   */
  protected async recordHistogram(name: string, value: number, tags?: Record<string, string>): Promise<void> {
    if (this.metrics) {
      try {
        await this.metrics.recordHistogram(name, value, tags);
      } catch (error) {
        this.logger.warn('Failed to record histogram', { name, error: (error as Error).message });
      }
    }
  }
  ...
  protected isSensitiveHeader(headerName: string, sensitiveFields: string[] = []): boolean {
    const defaultSensitive = ['authorization', 'cookie', 'x-api-key', 'x-auth-token'];
