...
} from "../src/websocket/WebSocketMiddlewareChain";
import type {
  WebSocketContext,
...
const createTestMiddleware = (
  ...
  return vi.fn(async (context: WebSocketContext, next: () => Promise<void>) => {
    if (delay > 0) {
...
describe("WebSocketMiddlewareChain", () => {
  ...
  beforeEach(() => {
    logger = Logger.getInstance("TestChain");
    metrics = MetricsCollector.getInstance();
    chain = new WebSocketMiddlewareChain(logger, metrics);
  });
  ...
  describe("Retry Logic", () => {
    ...
    it("should fail after max retries", async () => {
      const alwaysFailMiddleware = vi.fn(async () => {
  ...
  describe("Metrics Collection", () => {
    it("should record execution metrics", async () => {
      const recordCounterSpy = vi.spyOn(metrics, "recordCounter");
      const recordTimerSpy = vi.spyOn(metrics, "recordTimer");
...
describe("Performance Tests", () => {
  ...
  beforeEach(() => {
    const logger = Logger.getInstance("PerfTest");
