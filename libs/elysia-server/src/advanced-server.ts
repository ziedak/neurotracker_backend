...
export class AdvancedElysiaServerBuilder {
  ...
  /**
   * Get WebSocket middleware count (helper method since WebSocketMiddlewareChain doesn't expose count)
   */
  private getWebSocketMiddlewareCount(): number {
    if (!this.wsChain) return 0;
    // Access the middleware map size through a workaround
    return (this.wsChain as any).middleware?.size || 0;
  }
      ...
      if (patternConfig) {
        ...
        if (
          ...
          middlewares.push({
            ...
            priority: (patternConfig.priorities as any).logging ?? 150,
            enabled: true,
          });
        }
        ...
        if (
          ...
          middlewares.push({
            ...
            priority: (patternConfig.priorities as any).cors ?? 140,
            enabled: true,
          });
        }
        ...
        if (
          ...
          middlewares.push({
            ...
            priority: (patternConfig.priorities as any).security ?? 130,
            enabled: true,
          });
        }
        ...
        if (
          ...
          middlewares.push({
            ...
            priority: (patternConfig.priorities as any).metrics ?? 10,
            enabled: true,
          });
        }
      }
      ...
      if (patternConfig) {
        ...
        if (
          ...
          chain.register(
            {
              name: "ws-security",
              priority: (patternConfig.priorities as any).validation ?? 120,
            },
  ...
  private setupHttpMiddleware(): void {
    ...
    this.app!.onBeforeHandle(async ({ request, set }) => {
      ...
      try {
        await chainMiddleware(context, async () => {
    ...
    this.app.ws("/ws", {
      open: async (ws) => {
        ...
        if ("recordMetric" in this.metrics) {
          await (this.metrics as any).recordMetric(
      ...
      message: async (ws, message) => {
        ...
        const context: WebSocketContext = {
          ...
          metadata: {
            connectedAt: (ws as any).connectedAt || new Date(),
  ...
  private getHealthStatus() {
    ...
    return {
      status: "healthy",
      timestamp: new Date().toISOString(),
      uptime: now - ((this.app as any)?.startTime || now),
      server: {
        name: this.config.name,
        port: this.config.port,
