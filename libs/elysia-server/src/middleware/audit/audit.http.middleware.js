...
const DEFAULT_AUDIT_OPTIONS = {
    ...
    SKIP_ROUTES: ["/health", "/metrics", "/docs"],
 ...
 */
export class AuditHttpMiddleware extends BaseMiddleware {
    redisClient;
    clickhouseClient;
    constructor(metrics, config = {}, redisClient, clickhouseClient) {
        ...
        const completeConfig = {
            ...
            includeResponse: config.includeResponse ?? DEFAULT_AUDIT_OPTIONS.INCLUDE_RESPONSE,
            sensitiveFields: config.sensitiveFields ?? DEFAULT_AUDIT_OPTIONS.SENSITIVE_FIELDS,
            skipRoutes: config.skipRoutes ?? DEFAULT_AUDIT_OPTIONS.SKIP_ROUTES,
            storageStrategy: config.storageStrategy ?? DEFAULT_AUDIT_OPTIONS.STORAGE_STRATEGY,
            ...
            retentionDays: config.retentionDays ?? DEFAULT_AUDIT_OPTIONS.RETENTION_DAYS,
            anonymizePersonalData: config.anonymizePersonalData ??
                DEFAULT_AUDIT_OPTIONS.ANONYMIZE_PERSONAL_DATA,
            complianceMode: config.complianceMode ?? DEFAULT_AUDIT_OPTIONS.COMPLIANCE_MODE,
        };
    ...
    shouldSkip(context) {
        ...
        return (this.config.skipRoutes?.some((route) => path.includes(route)) ?? false);
    }
    /**
    ...
    extractContextInfo(context, extraInfoContext) {
        ...
        if ("action" in context && "resource" in context && extraInfoContext) {
            const auditEvent = extraInfoContext;
            if (auditEvent &&
    ...
    async storeInRedis(event) {
        ...
        await redis.setex(redisKey, this.config.redisTtl ?? 3600, // Default 1 hour - guaranteed by constructor
        JSON.stringify(this.sanitizeAuditEvent(event)));
    }
    /**
     * Store audit event in ClickHouse for analytics
     */
    async storeInClickHouse(event) {
    ...
    async getAuditSummary(startDate, endDate) {
        ...
        try {
            ...
            const [summaryResults, actionsResults, resourcesResults] = (await Promise.all([
            ...
            const toNumber = (value) => typeof value === "number" ? value : parseInt(value, 10);
            const toFloat = (value) => typeof value === "number" ? value : parseFloat(value);
            return {
                ...
                topResources: resourcesResults.map((row) => ({
    ...
    buildMetadata(context) {
        const metadata = {
            ...
            headers: this.sanitizeObject(context.request?.headers ?? {}, this.config.sensitiveFields),
        };
    ...
    sanitizeRequestBody(body) {
        if (!body || typeof body !== "object")
            return body;
        try {
            const sanitized = this.sanitizeObject(body, this.config.sensitiveFields);
     ...
     */
    sanitizeResponseBody(response) {
        if (!response || typeof response !== "object")
            return response;
        try {
            const sanitized = this.sanitizeObject(response, this.config.sensitiveFields);
    ...
    validateConfiguration() {
        ...
        if (!["redis", "clickhouse", "both"].includes(this.config.storageStrategy)) {
            throw new Error("Audit storageStrategy must be one of: redis, clickhouse, both");
        }
    }
    /**
...
//# sourceMappingURL=audit.http.middleware.js.map