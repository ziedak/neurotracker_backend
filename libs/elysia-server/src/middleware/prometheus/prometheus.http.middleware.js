 ...
 */
export class PrometheusHttpMiddleware extends BaseMiddleware {
    serviceName;
    metricsEndpoint;
    ...
    validateConfiguration(config) {
        ...
        if (config.buckets) {
            ...
            if (config.buckets.some((bucket) => typeof bucket !== "number" || bucket <= 0)) {
                throw new Error("Prometheus buckets must contain only positive numbers");
            }
        }
        ...
        if (config.excludePaths) {
            ...
            if (config.excludePaths.some((path) => typeof path !== "string" || !path.startsWith("/"))) {
        ...
        if (config.metricPrefix) {
            ...
            if (!prometheusNameRegex.test(config.metricPrefix)) {
                throw new Error("Prometheus metricPrefix must match Prometheus naming conventions");
            }
        }
    }
    /**
     ...
     */
    async handleMetricsEndpoint(context) {
        try {
     ...
     */
    async recordRequestMetrics(context, startTime, result) {
        ...
        if (this.config.includeUserAgentLabels &&
        ...
        if (this.config.trackUserMetrics &&
     ...
     */
    async recordDetailedMetrics(context) {
        const metricPrefix = this.config.metricPrefix || "";
        const requestSize = this.getRequestSize(context);
        const responseSize = this.getResponseSize(context);
        const path = this.normalizePath(this.getPathFromUrl(context.request.url));
    ...
    async recordNodeMetrics() {
        try {
                ...
                try {
                    ...
                    if (promClient?.register) {
                        ...
                        if (isBun) {
                            ...
                            this.logger.debug("Running on Bun, using Bun-compatible metrics collection");
                    ...
                    this.logger.debug("prom-client not available, skipping default metrics collection");
                }
            }
            ...
            this.logger.warn("Failed to collect runtime metrics", error);
        }
    }
    /**
    ...
    async recordBunMetrics() {
        try {
            ...
            if (typeof process !== "undefined" && process.memoryUsage) {
                ...
                await this.recordGauge("elysia_runtime_memory_usage_bytes", memUsage.rss || 0, {
                    service: this.serviceName,
                    runtime: "bun",
                    type: "rss",
                });
                if (memUsage.heapUsed) {
                    await this.recordGauge("elysia_runtime_memory_usage_bytes", memUsage.heapUsed, {
                        service: this.serviceName,
                        runtime: "bun",
                        type: "heap_used",
                    });
                ...
                if (memUsage.heapTotal) {
                    await this.recordGauge("elysia_runtime_memory_usage_bytes", memUsage.heapTotal, {
                        service: this.serviceName,
                        runtime: "bun",
                        type: "heap_total",
                    });
            ...
            if (typeof process !== "undefined" && process.hrtime) {
                ...
                setImmediate(() => {
                    ...
                    this.recordGauge("elysia_event_loop_lag_seconds", lag / 1000, {
                        ...
                        this.logger.warn("Failed to record Bun event loop lag", error);
                    });
                });
            }
            ...
            this.logger.warn("Failed to record Bun metrics", error);
        }
    }
    /**
     * Record Node.js-specific metrics
     */
    async recordNodeJSMetrics() {
        try {
            if (typeof process !== "undefined" && process.memoryUsage) {
                const memUsage = process.memoryUsage();
                // Memory metrics
                await this.recordGauge("elysia_runtime_memory_usage_bytes", memUsage.rss, {
                    service: this.serviceName,
                    runtime: "nodejs",
                    type: "rss",
                });
                ...
                await this.recordGauge("elysia_runtime_memory_usage_bytes", memUsage.external, {
                    service: this.serviceName,
                    runtime: "nodejs",
                    type: "external",
                });
                ...
                if (process.cpuUsage) {
                    ...
                    await this.recordGauge("elysia_runtime_cpu_usage_seconds", cpuUsage.user / 1000000, {
                        service: this.serviceName,
                        runtime: "nodejs",
                        type: "user",
                    });
                    await this.recordGauge("elysia_runtime_cpu_usage_seconds", cpuUsage.system / 1000000, {
                        service: this.serviceName,
                        runtime: "nodejs",
                        type: "system",
                    });
            ...
            if (process.hrtime) {
                ...
                setImmediate(() => {
                    ...
                    this.recordGauge("elysia_event_loop_lag_seconds", lag / 1000, {
                        ...
                        this.logger.warn("Failed to record Node.js event loop lag", error);
                    });
                });
            }
            ...
            this.logger.warn("Failed to record Node.js metrics", error);
        }
    }
    /**
     * Determine if Node.js metrics should be sampled
     */
    shouldSampleNodeMetrics() {
    ...
    async recordError(error, context) {
        ...
        if (this.config.includeStatusLabels !== false) {
            errorLabels["status"] = (context.set?.status ||
                context.response?.status ||
                500).toString();
        }
     ...
     */
    static createHighCardinalityConfig() {
        return {
            collectHttpMetrics: true,
            collectDefaultMetrics: true,
            enableDetailedMetrics: true,
...
//# sourceMappingURL=prometheus.http.middleware.js.map