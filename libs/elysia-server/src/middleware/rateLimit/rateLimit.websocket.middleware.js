 ...
 */
class WebSocketIpStrategy {
    generateKey(context) {
        return context.metadata.clientIp || "unknown";
    }
}
class WebSocketUserStrategy {
    generateKey(context) {
...
const DEFAULT_WEBSOCKET_RATE_LIMIT_OPTIONS = {
    ...
    KEY_STRATEGY: "connectionId",
    COUNT_MESSAGE_TYPES: [], // Empty means count all
    ...
    MESSAGES: {
        RATE_LIMIT_EXCEEDED: "Rate limit exceeded. Please slow down your message frequency.",
 ...
 */
export class RateLimitWebSocketMiddleware extends BaseWebSocketMiddleware {
    rateLimiter;
    connectionLimiter;
    strategies;
    cacheService;
    constructor(metrics, config = {}) {
        ...
        const completeConfig = {
            ...
            priority: config.priority ?? DEFAULT_WEBSOCKET_RATE_LIMIT_OPTIONS.PRIORITY,
            skipMessageTypes: config.skipMessageTypes ?? [],
            algorithm: config.algorithm ?? DEFAULT_WEBSOCKET_RATE_LIMIT_OPTIONS.ALGORITHM,
            ...
            windowMs: config.windowMs ?? DEFAULT_WEBSOCKET_RATE_LIMIT_OPTIONS.WINDOW_MS,
            keyStrategy: config.keyStrategy ?? DEFAULT_WEBSOCKET_RATE_LIMIT_OPTIONS.KEY_STRATEGY,
            customKeyGenerator: config.customKeyGenerator,
            countMessageTypes: config.countMessageTypes ??
            ...
            enableConnectionLimiting: config.enableConnectionLimiting ??
                DEFAULT_WEBSOCKET_RATE_LIMIT_OPTIONS.ENABLE_CONNECTION_LIMITING,
            closeOnLimit: config.closeOnLimit ??
                DEFAULT_WEBSOCKET_RATE_LIMIT_OPTIONS.CLOSE_ON_LIMIT,
            sendWarningMessage: config.sendWarningMessage ??
            ...
            message: {
                rateLimitExceeded: config.message?.rateLimitExceeded ??
                    DEFAULT_WEBSOCKET_RATE_LIMIT_OPTIONS.MESSAGES.RATE_LIMIT_EXCEEDED,
                connectionLimitExceeded: config.message?.connectionLimitExceeded ??
                    DEFAULT_WEBSOCKET_RATE_LIMIT_OPTIONS.MESSAGES
                        .CONNECTION_LIMIT_EXCEEDED,
                warningMessage: config.message?.warningMessage ??
        ...
        this.rateLimiter = new RateLimitingCacheAdapter(this.cacheService, {}, messageAdapterConfig);
        // Initialize connection rate limiter if enabled
        if (completeConfig.enableConnectionLimiting) {
            const connectionAdapterConfig = {
                ...(completeConfig.algorithm
                    ? { defaultAlgorithm: completeConfig.algorithm }
                    : {}),
            ...
            this.connectionLimiter = new RateLimitingCacheAdapter(this.cacheService, {}, connectionAdapterConfig);
        }
    ...
    async execute(context, next) {
        ...
        try {
            ...
            if (this.config.enableConnectionLimiting) {
                const connectionAllowed = await this.checkConnectionLimit(context, requestId);
            ...
            if (!result.allowed) {
                await this.handleRateLimitExceeded(context, result, key, requestId, "message");
                return;
            }
            ...
            this.logger.error("WebSocket rate limit middleware error", error, {
                requestId,
                connectionId: context.connectionId,
            ...
            await this.recordMetric("websocket_rate_limit_execution_time", executionTime, {
                algorithm: this.config.algorithm,
                keyStrategy: this.config.keyStrategy,
                connectionId: context.connectionId,
            });
     ...
     */
    async checkConnectionLimit(context, requestId) {
        ...
        try {
            ...
            const result = await this.connectionLimiter.checkRateLimit(ipKey, this.config.maxConnectionsPerIP, this.config.windowMs, this.config.algorithm);
            if (!result.allowed) {
                await this.handleRateLimitExceeded(context, result, ipKey, requestId, "connection");
                return false;
            }
            return true;
        }
        catch (error) {
            this.logger.error("Connection rate limit check failed", error, {
                requestId,
                connectionId: context.connectionId,
     ...
     */
    async checkMessageRateLimit(key) {
        const adapterResult = await this.rateLimiter.checkRateLimit(key, this.config.maxMessagesPerMinute, this.config.windowMs, this.config.algorithm);
     ...
     */
    async handleRateLimitExceeded(context, result, key, requestId, limitType) {
        ...
        const errorMessage = limitType === "connection"
        ...
        if (this.config.closeOnLimit) {
            ...
            try {
                const closeResult = asWebSocket(context.ws).close(1008, `Rate limit exceeded: ${errorMessage}`);
                if (closeResult && typeof closeResult.then === "function") {
                    closeResult.catch((err) => this.logger.error("WebSocket close failed:", err));
                }
            }
            catch (closeError) {
     ...
     */
    async checkAndSendWarning(context, result) {
     ...
     */
    sendErrorMessage(context, errorData) {
        try {
            if (asWebSocket(context.ws).readyState === 1) {
                // WebSocket.OPEN
                const sendResult = asWebSocket(context.ws).send(JSON.stringify(errorData));
     ...
     */
    sendWarningMessage(context, warningData) {
        try {
            if (asWebSocket(context.ws).readyState === 1) {
                // WebSocket.OPEN
                const sendResult = asWebSocket(context.ws).send(JSON.stringify(warningData));
                if (sendResult && typeof sendResult.then === "function") {
                    sendResult.catch((err) => this.logger.error("WebSocket send failed:", err));
                }
            }
    ...
    generateKey(context) {
        ...
        if (!strategy) {
            this.logger.warn("Unknown WebSocket rate limit strategy, falling back to connectionId", {
                strategy: this.config.keyStrategy,
                connectionId: context.connectionId,
            });
            return (this.strategies.get("connectionId")?.generateKey(context) ?? "unknown");
        }
    ...
    shouldCountMessage(messageType) {
        ...
        if (this.config.excludeMessageTypes &&
    ...
    initializeCacheService() {
        try {
            ...
            this.logger.error("Failed to create CacheService", error);
            throw new Error(`CacheService initialization failed: ${error.message}`);
        }
    }
    /**
    ...
    async recordRateLimitMetrics(action, additionalTags = {}) {
        await this.recordMetric(`websocket_rate_limit_${action}`, 1, additionalTags);
    }
    /**
     * Validate configuration on instantiation
     */
    validateConfiguration() {
        const { maxMessagesPerMinute, maxConnectionsPerIP, windowMs, keyStrategy, customKeyGenerator, warningThreshold, } = this.config;
        if (maxMessagesPerMinute <= 0 || !Number.isInteger(maxMessagesPerMinute)) {
            throw new Error("WebSocket rate limit maxMessagesPerMinute must be a positive integer");
        }
        if (maxConnectionsPerIP !== undefined &&
            (maxConnectionsPerIP <= 0 || !Number.isInteger(maxConnectionsPerIP))) {
            throw new Error("WebSocket rate limit maxConnectionsPerIP must be a positive integer");
        }
        if (windowMs <= 0 || !Number.isInteger(windowMs)) {
            throw new Error("WebSocket rate limit windowMs must be a positive integer");
        }
        if (keyStrategy === "custom" && !customKeyGenerator) {
            throw new Error("WebSocket rate limit customKeyGenerator is required when keyStrategy is 'custom'");
        }
        if (!["ip", "user", "connectionId", "custom"].includes(keyStrategy)) {
            throw new Error("WebSocket rate limit keyStrategy must be one of: ip, user, connectionId, custom");
        }
        if (warningThreshold !== undefined &&
            (warningThreshold < 0 || warningThreshold > 100)) {
            throw new Error("WebSocket rate limit warningThreshold must be between 0 and 100");
        }
    }
    /**
     * Reset rate limit for a specific key
     */
    async resetRateLimit(identifier, limitType = "message") {
        try {
            const limiter = limitType === "connection" ? this.connectionLimiter : this.rateLimiter;
            if (!limiter) {
            ...
            this.logger.error("Failed to reset WebSocket rate limit", error, {
                identifier,
                limitType,
            });
            throw error;
    ...
    getRateLimitStats() {
        try {
            ...
            this.logger.error("Failed to get WebSocket rate limit stats", error);
    ...
    async getHealth() {
        try {
            ...
            this.logger.error("Failed to get WebSocket rate limit health", error);
            return {
                messageRateLimiter: {
                    healthy: false,
                    error: error instanceof Error ? error.message : "unknown",
 ...
 */
export function createRateLimitWebSocketMiddleware(metrics, config) {
...
export const WEBSOCKET_RATE_LIMIT_PRESETS = {
    general: () => RateLimitWebSocketMiddleware.createGeneralConfig(),
    strict: () => RateLimitWebSocketMiddleware.createStrictConfig(),
    gaming: () => RateLimitWebSocketMiddleware.createGamingConfig(),
    chat: () => RateLimitWebSocketMiddleware.createChatConfig(),
    development: () => RateLimitWebSocketMiddleware.createDevelopmentConfig(),
    production: () => RateLimitWebSocketMiddleware.createProductionConfig(),
};
//# sourceMappingURL=rateLimit.websocket.middleware.js.map