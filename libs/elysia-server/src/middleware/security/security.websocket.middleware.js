 ...
 */
export class SecurityWebSocketMiddleware extends BaseWebSocketMiddleware {
    ...
    constructor(metrics, config) {
        ...
        const defaultConfig = config.customValidation
    ...
    async execute(context, next) {
        ...
        try {
            ...
            this.logger.error("WebSocket security middleware error", error, {
                connectionId: context.connectionId,
                messageType: context.message?.type || "unknown",
                clientIp: context.metadata?.clientIp || "unknown",
            });
            // Record security violation
            await this.recordSecurityViolation(context, error);
            throw error;
        }
        finally {
            await this.recordTimer("websocket_security_duration", Date.now() - startTime, {
                messageType: context.message?.type || "unknown",
            });
        }
    }
     ...
     */
    async registerConnection(context) {
     ...
     */
    validateMessageSecurity(context) {
        ...
        if (this.config.messageTypeWhitelist &&
     ...
     */
    sanitizeMessagePayload(context) {
     ...
     * Record security metrics
     */
    async recordSecurityMetrics(context) {
        await this.recordMetric("websocket_security_message_processed", 1, {
            messageType: context.message?.type || "unknown",
            origin: this.getOrigin(context),
            clientIp: context.metadata.clientIp,
        });
        await this.recordHistogram("websocket_security_message_size", this.calculateMessageSize(context.message), {
            messageType: context.message?.type || "unknown",
        });
    }
    /**
     * Record security violation
     */
    async recordSecurityViolation(context, error) {
        const connectionInfo = this.connectionRegistry.get(context.connectionId);
        if (connectionInfo) {
    ...
    isSecureConnection(context) {
        ...
        return (context.metadata.headers["x-forwarded-proto"] === "https" ||
            context.metadata.headers["origin"]?.startsWith("https://") ||
            process.env["NODE_ENV"] === "development");
    }
    isProtocolAllowed(context) {
        if (!this.config.allowedProtocols ||
    ...
    getOrigin(context) {
        ...
        if (context.metadata?.headers) {
            return (context.metadata.headers["origin"] ??
        ...
        if (testContext.request?.headers) {
            return (testContext.request.headers["origin"] ??
    ...
    shouldSkip(context) {
        ...
        return (this.config.skipMessageTypes?.some((skipType) => {
    ...
    static createHighSecurityConfig() {
        return {
            allowedOrigins: [], // Must be explicitly configured
    ...
    static createHighSecurity(metrics, additionalConfig) {
        const highSecConfig = SecurityWebSocketMiddleware.createHighSecurityConfig();
        const config = {
            ...highSecConfig,
            ...additionalConfig,
            enabled: true,
    ...
    static createApiGateway(metrics, additionalConfig) {
 ...
 */
export function createSecurityWebSocketMiddleware(metrics, config) {
    const finalConfig = {
        enabled: true,
...
//# sourceMappingURL=security.websocket.middleware.js.map