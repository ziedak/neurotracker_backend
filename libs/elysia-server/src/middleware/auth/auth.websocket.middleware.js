...
const DEFAULT_WS_AUTH_OPTIONS = {
    ...
    SKIP_AUTH_MESSAGE_TYPES: ["ping", "pong", "heartbeat"],
 ...
 */
export class AuthWebSocketMiddleware extends BaseWebSocketMiddleware {
    authService;
    authenticatedConnections = new Map();
    scheduler = Scheduler.create();
    static CLEANUP_TIMER_KEY = "ws-auth-cleanup";
    constructor(metrics, authService, config = {}) {
        ...
        const completeConfig = {
            ...
            allowAnonymous: config.allowAnonymous ?? DEFAULT_WS_AUTH_OPTIONS.ALLOW_ANONYMOUS,
            skipAuthMessageTypes: config.skipAuthMessageTypes ??
                DEFAULT_WS_AUTH_OPTIONS.SKIP_AUTH_MESSAGE_TYPES,
            skipMessageTypes: [
                ...(config.skipMessageTypes ?? []),
                ...(config.skipAuthMessageTypes ?? []),
            ],
            closeOnAuthFailure: config.closeOnAuthFailure ??
            ...
            extractUserInfo: config.extractUserInfo ?? DEFAULT_WS_AUTH_OPTIONS.EXTRACT_USER_INFO,
    ...
    async execute(context, next) {
        ...
        try {
            ...
            if (this.config.allowUnauthenticatedTypes?.includes(context.message.type)) {
            ...
            this.logger.error("WebSocket authentication middleware error", error, {
                requestId,
                messageType: context.message.type,
                connectionId: context.connectionId,
            ...
            await this.handleAuthenticationFailure(context, error);
            throw error;
        }
        finally {
     ...
     */
    async authenticateConnection(context, requestId) {
     ...
     */
    async performAuthentication(context, requestId) {
        const results = [];
     ...
     */
    async authorizeMessage(context, authResult, requestId) {
        ...
        if (messageRoles && messageRoles.length > 0) {
            const hasMessageRole = messageRoles.some((role) => user?.roles.includes(role));
            if (!hasMessageRole) {
                await this.recordAuthMetrics("ws_auth_failure", {
                ...
                throw new ForbiddenError(`Insufficient role permissions for message type: ${messageType}`);
            }
        }
        ...
        if (messagePermissions && messagePermissions.length > 0) {
            const hasMessagePermission = messagePermissions.some((permission) => user.permissions.includes(permission));
            if (!hasMessagePermission) {
                ...
                throw new ForbiddenError(`Insufficient permissions for message type: ${messageType}`);
            }
        }
        ...
        if (messageAction) {
            const canPerformMessage = this.authService.can(user, messageAction.action, messageAction.resource);
            if (!canPerformMessage) {
                ...
                throw new ForbiddenError(`Insufficient permissions for message action: ${messageType}`);
            }
        }
     ...
     */
    isAuthenticationValid(connection) {
        const now = new Date();
        const timeSinceAuth = now.getTime() - connection.authenticatedAt.getTime();
        const timeSinceActivity = now.getTime() - connection.lastActivity.getTime();
        const reauthInterval = this.config.reauthenticationInterval ??
            DEFAULT_WS_AUTH_OPTIONS.REAUTHENTICATION_INTERVAL;
        const authTimeout = this.config.authenticationTimeout ??
            DEFAULT_WS_AUTH_OPTIONS.AUTHENTICATION_TIMEOUT;
     ...
     * Handle authentication failures
     */
    async handleAuthenticationFailure(context, error) {
        if (this.config.closeOnAuthFailure &&
            (error instanceof UnauthorizedError || error instanceof ForbiddenError)) {
            this.logger.warn("Closing WebSocket connection due to authentication failure", {
                connectionId: context.connectionId,
                messageType: context.message.type,
                error: error.message,
            });
            ...
            // Close connection
            try {
                await asWebSocket(context.ws).close(error instanceof UnauthorizedError ? 4401 : 4403, error.message);
            }
            catch (closeError) {
                this.logger.error("Failed to close WebSocket connection", closeError, {
                    connectionId: context.connectionId,
                });
            }
     ...
     */
    startCleanupTimer() {
        this.scheduler.setInterval(AuthWebSocketMiddleware.CLEANUP_TIMER_KEY, 300000, // Cleanup every 5 minutes
        () => {
    ...
    cleanupExpiredConnections() {
        ...
        if (expiredConnections.length > 0) {
            this.logger.debug("Cleaned up expired WebSocket authentication sessions", {
    ...
    validateConfiguration() {
        ...
        if (reauthInterval < authTimeout) {
            throw new Error("Reauthentication interval must be greater than authentication timeout");
        }
    }
    /**
     ...
     */
    static createMessagePermissionConfig(messagePermissions) {
        return {
 ...
 */
export function createAuthWebSocketMiddleware(metrics, authService, config) {
...
export const WS_AUTH_PRESETS = {
    requireAuth: () => AuthWebSocketMiddleware.createRequireAuthConfig(),
    optionalAuth: () => AuthWebSocketMiddleware.createOptionalAuthConfig(),
    adminOnly: () => AuthWebSocketMiddleware.createRoleBasedConfig(["admin"]),
    userOrAdmin: () => AuthWebSocketMiddleware.createRoleBasedConfig(["user", "admin"]),
    realtimeChat: () => ({
...
//# sourceMappingURL=auth.websocket.middleware.js.map