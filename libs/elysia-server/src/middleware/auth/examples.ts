 ...
 * Comprehensive examples demonstrating AuthHttpMiddleware integration and configuration
 */


import { type AuthenticationService } from "@libs/auth";
import { type IMetricsCollector } from "@libs/monitoring";
import { AUTH_PRESETS, createAuthHttpMiddleware, type AuthHttpMiddleware, type AuthHttpMiddlewareConfig } from "./auth.http.middleware";
...
export function createBasicAuthExample(
  ...
  return createAuthHttpMiddleware(metrics, authService, {
    name: "basic-auth",
    requireAuth: true,
    jwtAuth: true,
    apiKeyAuth: false,
...
export function createDevelopmentExample(
  ...
  return createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.development());
}
...
export function createProductionExample(
  ...
  return createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.production());
}
...
export function createCustomStrategyExample(
  ...
  const customConfig: Partial<AuthHttpMiddlewareConfig> = {
  ...
  return createAuthHttpMiddleware(metrics, authService, customConfig);
}
...
export function createConditionalAuthExample(
  ...
  const getConfigForEnvironment = (
    env: string
  ): Partial<AuthHttpMiddlewareConfig> => {
    switch (env) {
      case "production":
        return {
  ...
  return createAuthHttpMiddleware(
    metrics,
    authService,
...
export const CONFIG_EXAMPLES = {
  ...
  requireAuth: (
    metrics: IMetricsCollector,
    authService: AuthenticationService
  ) => createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.requireAuth()),

  // Optional authentication with user context extraction
  optionalAuth: (
    metrics: IMetricsCollector,
    authService: AuthenticationService
  ) => createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.optionalAuth()),

  // Admin-only access with role checking
  adminOnly: (metrics: IMetricsCollector, authService: AuthenticationService) =>
    createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.adminOnly()),

  // User or admin access with multiple roles
  userOrAdmin: (
    metrics: IMetricsCollector,
    authService: AuthenticationService
  ) => createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.userOrAdmin()),

  // API access with API key authentication only
  apiAccess: (metrics: IMetricsCollector, authService: AuthenticationService) =>
    createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.apiAccess()),

  // Web application with session and JWT support
  webApp: (metrics: IMetricsCollector, authService: AuthenticationService) =>
    createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.webApp()),

  // Development environment with relaxed security
  development: (
    metrics: IMetricsCollector,
    authService: AuthenticationService
  ) => createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.development()),

  // Production environment with strict security
  production: (
    metrics: IMetricsCollector,
    authService: AuthenticationService
  ) => createAuthHttpMiddleware(metrics, authService, AUTH_PRESETS.production()),
} as const;
...
export const USAGE_PATTERNS = {
  ...
  layered: (metrics: IMetricsCollector, authService: AuthenticationService) => {
    const publicAuth = createAuthHttpMiddleware(metrics, authService, {
      name: "public-auth",
      requireAuth: false,
      allowAnonymous: true,
      extractUserInfo: true,
    ...
    const protectedAuth = createAuthHttpMiddleware(metrics, authService, {
      name: "protected-auth",
      requireAuth: true,
      allowAnonymous: false,
    });

    const adminAuth = createAuthHttpMiddleware(metrics, authService, {
      name: "admin-auth",
      requireAuth: true,
      roles: ["admin"],
      allowAnonymous: false,
  ...
  routeSpecific: (
    ...
    const readOnlyAuth = createAuthHttpMiddleware(metrics, authService, {
      name: "read-only",
      requireAuth: true,
      permissions: ["read"],
      action: "read",
    ...
    const writeAuth = createAuthHttpMiddleware(metrics, authService, {
      name: "write-access",
      requireAuth: true,
      permissions: ["write"],
      action: "update",
    ...
    const deleteAuth = createAuthHttpMiddleware(metrics, authService, {
      name: "delete-access",
      requireAuth: true,
      permissions: ["delete"],
      roles: ["admin"],
  ...
  environmentBased: (
    ...
    return createAuthHttpMiddleware(metrics, authService, config);
  },
} as const;
...
export const TESTING_UTILS = {
  ...
  createMockMiddleware: () => {
    ...
    const middleware = createAuthHttpMiddleware(mockMetrics, mockAuthService, {
      name: "test-auth",
      requireAuth: true,
      jwtAuth: true,
    });
