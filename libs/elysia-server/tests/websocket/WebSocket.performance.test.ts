...
import type { WebSocketContext } from "../src/middleware/src/types";
...
// Setup mocks
beforeEach(() => {
  jest.mocked(Logger).getInstance = jest.fn(() => mockLogger);
  jest.mocked(MetricsCollector).getInstance = jest.fn(() => mockMetrics);
...
describe("WebSocket Middleware Performance Benchmarks", () => {
    ...
    it("should handle middleware registration and unregistration efficiently", async () => {
      ...
      for (let i = 0; i < middlewareCount; i++) {
        ...
        const middleware = jest.fn(
          async (_context: WebSocketContext, next: () => Promise<void>) => {
            await next();
          }
        );
  ...
  describe("Error Recovery Performance", () => {
    it("should recover quickly from circuit breaker states", async () => {
      ...
      await expect(executor(context, jest.fn())).rejects.toThrow();
      await expect(executor(context, jest.fn())).rejects.toThrow();
      ...
      await executor(context, jest.fn()); // This should succeed and close the circuit
      ...
      await executor(context, jest.fn());
