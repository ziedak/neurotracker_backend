 ...
 */
import { CacheOperationLockManager, } from "@libs/database";
import { createLogger } from "@libs/utils";
/**
 * Default configuration optimized for rate limiting
 */
export const DEFAULT_RATE_LIMITING_ADAPTER_CONFIG = {
...
export class RateLimitingCacheAdapter {
    ...
    stats;
    lockManager;
    logger = createLogger("RateLimitingCacheAdapter");
    constructor(cacheService, configValidator, config = {}) {
     ...
     */
    async checkRateLimit(identifier, limit, windowMs, algorithm = this.config.defaultAlgorithm) {
        const startTime = performance.now();
        const key = this.generateCacheKey(identifier, algorithm, windowMs);
        try {
            // Use lock manager to prevent race conditions
            const result = await this.lockManager.acquireLock(key, "rate-limit-check", async () => {
                return await this.executeRateLimitCheck(key, identifier, limit, windowMs, algorithm);
     ...
     */
    async checkMultipleRateLimits(requests) {
        ...
        const batchPromises = requests.map(async (request) => {
            try {
                const result = await this.checkRateLimit(request.identifier, request.limit, request.windowMs, request.algorithm);
                if (result.cached)
                    cacheHits++;
                return result;
            }
     ...
     */
    async executeRateLimitCheck(key, _identifier, // Prefixed with _ to indicate intentionally unused
    limit, windowMs, algorithm) {
        switch (algorithm) {
            case "fixed-window":
    ...
    async checkFixedWindow(key, limit, windowMs) {
        ...
        const cacheResult = await this.cacheService.get(windowKey);
    ...
    async checkSlidingWindow(key, limit, windowMs) {
        ...
        const validEntries = slidingData.filter((timestamp) => timestamp > windowStart);
        ...
        const resetTime = validEntries.length > 1
            ? (validEntries[0] || now) + windowMs
            : now + windowMs;
        return {
            allowed: true,
     ...
     */
    async checkTokenBucket(key, limit, windowMs) {
        const now = Date.now();
     ...
     */
    async checkLeakyBucket(key, limit, windowMs) {
        const now = Date.now();
        ...
        if (currentVolume >= limit) {
            return {
                ...
                limit,
                remaining: 0,
                resetTime: Date.now() + Math.ceil((currentVolume - limit + 1) / leakRate),
     ...
     */
    generateCacheKey(identifier, algorithm, windowMs) {
     ...
     */
    alignToFixedWindow(key, windowMs) {
        const now = Date.now();
     ...
     */
    updateStatistics(result, responseTime, algorithm) {
        ...
        if (result.cached) {
            const hitRate = (this.stats.cacheHitRate * (this.stats.totalRequests - 1) + 1) /
                this.stats.totalRequests;
    ...
    updateMemoryStats() {
        ...
        this.stats.memoryUsage = {
            ...
            peakMB: Math.max(this.stats.memoryUsage.peakMB, stats.memoryUsage / (1024 * 1024)),
            utilizationPercent: Math.min(100, (stats.memoryUsage / (50 * 1024 * 1024)) * 100), // Assume 50MB max
        };
    }
    /**
    ...
    validateConfiguration() {
        ...
        if (!validation.valid) {
            this.logger.error("Rate limiting adapter configuration validation failed", {
                errors: validation.errors,
            });
            throw new Error(`Invalid rate limiting adapter configuration: ${validation.errors.join(", ")}`);
        }
     ...
     */
    async resetRateLimit(identifier, algorithm) {
        const keyPattern = algorithm
            ? `${this.config.keyPrefix}${algorithm}:*:${identifier.replace(/[^a-zA-Z0-9_-]/g, "_")}`
            : `${this.config.keyPrefix}*:${identifier.replace(/[^a-zA-Z0-9_-]/g, "_")}`;
     ...
     */
    async warmupRateLimitKeys(identifiers) {
        const warmupPromises = identifiers.map(async (identifier) => {
            const key = this.generateCacheKey(identifier, this.config.defaultAlgorithm, 60000);
...
//# sourceMappingURL=RateLimitingCacheAdapter.js.map