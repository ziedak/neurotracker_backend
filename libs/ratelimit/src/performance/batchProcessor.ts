...
import { RateLimitResult } from "../redisRateLimit";
...
export class BatchRateLimitProcessor {
  ...
  constructor(
    ...
    private readonly scriptManager: SharedScriptManager,
    private readonly logger: ILogger,
    private readonly keyPrefix: string = "rate_limit",
    private readonly ttlBuffer: number = 10,
    private readonly config: Partial<PipelineConfig> = {}
  ) {
    this.logger = logger.child({ component: "BatchRateLimitProcessor" });
  }
   ...
   */
  async processBatch(
    requests: BatchRateLimitRequest[],
    algorithm:
      | "sliding-window"
      | "token-bucket"
      | "fixed-window" = "sliding-window"
  ): Promise<BatchRateLimitResponse[]> {
    ...
    try {
      ...
      const results: BatchRateLimitResponse[] = [];
   ...
   */
  private async processSlidingWindowBatch(
    requests: BatchRateLimitRequest[]
  ): Promise<BatchRateLimitResponse[]> {
  ...
  private async processIndividualRequests(
    ...
    const scriptSha = this.scriptManager.getScriptSha(
      this.getScriptName(algorithm)
    );
    if (!scriptSha) {
    ...
    return requests.map((request, index) => {
      ...
      const scriptResult = pipelineResult[1] as number[];
  ...
  private getScriptName(
    algorithm: string
  ): "SLIDING_WINDOW" | "TOKEN_BUCKET" | "FIXED_WINDOW" {
    switch (algorithm) {
      case "sliding-window":
   ...
   */
  private validateBatchRequests(requests: BatchRateLimitRequest[]): void {
  ...
  private chunkRequests(
    ...
    const chunks: BatchRateLimitRequest[][] = [];
  ...
  private calculateBatchStats(
    results: BatchRateLimitResponse[],
    ...
    const errors = results.filter(
      (r) => !r.result.allowed && r.result.totalHits === -1
    ).length;
