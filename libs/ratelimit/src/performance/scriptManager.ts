...
export class SharedScriptManager {
   ...
   */
  async initialize(redisClient: RedisClient, logger: ILogger): Promise<void> {
    this.logger = logger.child({ component: "SharedScriptManager" });
  ...
  async forceReinitialize(redisClient: RedisClient): Promise<void> {
    this.reset();
    if (this.logger) {
      await this.initialize(redisClient, this.logger);
    }
  }
}
