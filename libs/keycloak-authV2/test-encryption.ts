...
import { createEncryptionManager } from "./src/services/EncryptionManager";
...
async function testEncryption() {
  ...
  const encryptionManager = createEncryptionManager("test-master-key-12345");
  ...
  try {
    ...
    console.log(
      `   ✅ Different: ${encrypted1 !== encrypted2 ? "YES" : "NO"}\n`
    );
    ...
    if (decrypted1 !== testToken || decrypted2 !== testToken) {
      throw new Error(
        "Different encrypted values should decrypt to same original!"
      );
    }
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `   Valid encrypted data: ${isValid ? "✅ VERIFIED" : "❌ FAILED"}`
    );
    console.log(
      `   Invalid encrypted data: ${
        isInvalid ? "❌ SHOULD FAIL" : "✅ CORRECTLY REJECTED"
      }\n`
    );
  ...
  try {
    const manager1 = createEncryptionManager("key1");
    const manager2 = createEncryptionManager("key2");
    ...
    try {
      ...
      console.log(
        `   ✅ Key isolation: Manager2 cannot decrypt Manager1 data\n`
      );
    }
  } catch (error) {
...
testEncryption()
  .then((success) => {
    if (success) {
      ...
      console.log(
        "✅ EncryptionManager successfully replaced vulnerable base64 encoding"
      );
