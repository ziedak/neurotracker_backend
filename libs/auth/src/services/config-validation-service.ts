  ...
  validateConfig(config: AuthConfig): ValidationResult {
    ...
    for (const rule of this.validationRules) {
      const error = rule.validator(
        this.getNestedValue(config, rule.field),
        config
      );
        ...
        validator: (value) => {
          if (!value || typeof value !== "string") {
            return {
              field: "jwt.refreshExpiresIn",
              message:
                "Refresh token expiration time is required and must be a string",
              severity: "error",
              suggestion:
                'Set refresh token expiration time (e.g., "7d", "30d")',
            };
          }
          return null;
        },
        ...
        validator: (value) => {
          if (!value || typeof value !== "string") {
            return {
              ...
              severity: "error",
              suggestion:
                "Set the Keycloak server URL (e.g., https://keycloak.example.com)",
            };
          }
          try {
            new URL(value);
        ...
        validator: (value) => {
          if (!value || typeof value !== "string") {
            return {
              field: "keycloak.clientSecret",
              message:
                "Keycloak client secret is required and must be a string",
              severity: "error",
          ...
          if (value.length < 16) {
            return {
              field: "keycloak.clientSecret",
              message:
                "Keycloak client secret should be at least 16 characters long",
              severity: "warning",
        ...
        validator: (value) => {
          ...
          if (value < 300) {
            return {
              field: "session.ttl",
              message:
                "Session TTL is very short, consider increasing for better UX",
              severity: "warning",
    ...
    if (config.jwt.expiresIn && config.jwt.refreshExpiresIn) {
      ...
      if (accessExpiry && refreshExpiry && accessExpiry >= refreshExpiry) {
        errors.push({
          ...
          severity: "error",
          suggestion:
            "Set access token expiry much shorter than refresh token expiry",
        });
      }
    }
    ...
    if (config.session.ttl && config.jwt.expiresIn) {
      ...
      if (jwtExpiry && sessionTtl > jwtExpiry * 2) {
        warnings.push({
          field: "session.ttl",
          message:
            "Session TTL is much longer than JWT expiry, consider aligning them",
 ...
 */
export function createConfigValidationService(): ConfigValidationService {
