...
export class EnhancedPermissionCacheService {
     ...
     */
    async setUserPermissions(userId, permissions, roles = [], ttl) {
        try {
            const cacheKey = `perm:user:${userId}`;
            const entry = {
                permissions,
     ...
     * Cache user roles
     */
    async setUserRoles(userId, roles, ttl) {
        try {
            const cacheKey = `roles:user:${userId}`;
            const entry = {
                permissions: [],
    ...
    async invalidateUserPermissions(userId) {
        try {
            ...
            this.deps.monitoring.logger.error("Failed to invalidate user permissions", {
                userId,
                error,
            });
        }
     ...
     */
    async invalidateRolePermissions(roleName) {
        try {
            ...
            this.deps.monitoring.logger.error("Failed to invalidate role permissions", {
                roleName,
                error,
            });
        }
     ...
     */
    startCleanupTimer() {
        this.cleanupTimer = setInterval(() => this.cleanup(), this.cacheConfig.cleanupInterval);
    }
    /**
 ...
 */
export function createEnhancedPermissionCacheService(deps, config) {
...
export function getCacheStatus(cache) {
    ...
    const isHealthy = stats.hitRate > 0.1 && stats.memoryUsage < 100 * 1024 * 1024; // 100MB limit
    return { isHealthy, stats, config };
}
export default EnhancedPermissionCacheService;
//# sourceMappingURL=enhanced-permission-cache-service.js.map