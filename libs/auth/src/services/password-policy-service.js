...
export class PasswordPolicyService {
    ...
    constructor(authConfig, deps) {
        ...
        this.config = {
            ...
            requireNumbers: authConfig.passwordPolicy?.requireNumbers ?? true,
            requireSpecialChars: authConfig.passwordPolicy?.requireSpecialChars ?? true,
            specialChars: authConfig.passwordPolicy?.specialChars || "!@#$%^&*()_+-=[]{}|;:,.<>?",
            blacklistedPasswords: authConfig.passwordPolicy?.blacklistedPasswords || [],
            enableCommonPasswordCheck: authConfig.passwordPolicy?.enableCommonPasswordCheck ?? true,
            enableCompromisedPasswordCheck: authConfig.passwordPolicy?.enableCompromisedPasswordCheck ?? false,
        };
    }
    /**
    ...
    async validatePassword(password, userContext) {
        ...
        if (userContext && this.containsPersonalInfo(password, userContext)) {
            ...
            suggestions.push("Avoid using your email, name, or other personal details");
     ...
     */
    checkComplexityRequirements(password) {
        return {
            length: password.length >= this.config.minLength,
            uppercase: /[A-Z]/.test(password),
            lowercase: /[a-z]/.test(password),
            numbers: /[0-9]/.test(password),
            specialChars: new RegExp(`[${this.escapeRegex(this.config.specialChars)}]`).test(password),
     ...
     */
    async validatePasswords(passwords) {
        const results = [];
    ...
    isSequentialPattern(password) {
        ...
        const lowerPassword = password.toLowerCase();
        return sequential.some((seq) => seq.includes(lowerPassword) && lowerPassword.length >= 4);
    }
    ...
    isBlacklistedPassword(password) {
        const lowerPassword = password.toLowerCase();
        return this.config.blacklistedPasswords.some((blacklisted) => lowerPassword === blacklisted.toLowerCase());
    }
    containsPersonalInfo(password, userContext) {
        const lowerPassword = password.toLowerCase();
    ...
    analyzePatternStrength(password) {
        let bonus = 0;
        ...
        const specialMatches = password.match(new RegExp(`[${this.escapeRegex(this.config.specialChars)}]`, "g"));
        if (specialMatches) {
 ...
 */
export function createPasswordPolicyService(config, deps) {
