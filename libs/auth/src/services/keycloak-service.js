...
export class KeycloakService {
    ...
    async initialize() {
        try {
            ...
            if (!adminUsername || !adminPassword) {
                throw new AuthError("Keycloak admin credentials not configured. Please set KEYCLOAK_ADMIN_USERNAME and KEYCLOAK_ADMIN_PASSWORD environment variables.", "KEYCLOAK_CONFIG_MISSING");
            }
            ...
            this.deps.monitoring.logger.info("Keycloak client initialized successfully");
        }
        catch (error) {
            this.deps.monitoring.logger.error("Failed to initialize Keycloak client", { error });
            throw new AuthError("Keycloak initialization failed", "KEYCLOAK_INIT_FAILED");
        }
    }
    /**
     ...
     */
    async authenticateUserEnhanced(email, password, options) {
        ...
        try {
            ...
            this.deps.monitoring.logger.error("Enhanced Keycloak authentication failed", {
                email,
                error: errorMessage,
    ...
    async registerUser(data) {
        try {
            ...
            if (!user) {
                throw new AuthError("Failed to retrieve created user", "USER_CREATION_FAILED");
            }
    ...
    async getUserPermissions(userId) {
        try {
            ...
            this.deps.monitoring.logger.error("Failed to get user permissions from Keycloak", { userId, error });
            return [];
        }
    }
    /**
    ...
    async getUserRoles(userId) {
        try {
            ...
            this.deps.monitoring.logger.error("Failed to get user roles from Keycloak", { userId, error });
            return [];
        }
    }
    /**
     ...
     */
    buildUserDisplayName(keycloakUser) {
     ...
     */
    async performDirectGrantAuthentication(username, password) {
        try {
 ...
 */
export function createKeycloakService(config, deps) {
    return new KeycloakService(config, deps);
}
/**
 * Initialize Keycloak service
 */
export async function initializeKeycloakService(service) {
