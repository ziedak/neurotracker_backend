...
export class SessionService {
    ...
    async updateSessionActivity(sessionId) {
        try {
            ...
            if (timeUntilExpiry < this.config.session.refreshThreshold * 1000) {
                session.expiresAt = new Date(Date.now() + this.config.session.ttl * 1000);
            }
            await this.storeSession(session);
            return true;
        }
    ...
    async storeSession(session) {
        ...
        await this.deps.redis.setex(`session:${session.id}`, ttl, JSON.stringify(session));
    }
}
// ===================================================================
// UTILITY FUNCTIONS
 ...
 */
export function createSessionService(config, deps) {
