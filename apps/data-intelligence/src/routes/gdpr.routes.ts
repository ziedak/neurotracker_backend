...
export function setupGdprRoutes(app: Elysia) {
  ...
  const logger = new Logger("gdpr");

  // Data Access Request
  app.get("/v1/gdpr/access/:userId", async ({ params }) => {
    try {
      ...
      return { error: "GDPR access error", message: (error as Error).message };
    }
  });

  // Data Erasure Request
  app.delete("/v1/gdpr/erase/:userId", async ({ params }) => {
    try {
      ...
      return { error: "GDPR erasure error", message: (error as Error).message };
    }
  });
  ...
  app.get("/v1/gdpr/export/:userId", async ({ params }) => {
    try {
      ...
      return { error: "GDPR export error", message: (error as Error).message };
    }
  });
}
