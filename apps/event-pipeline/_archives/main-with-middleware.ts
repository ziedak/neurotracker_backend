import { WebSocketGateway } from "../src/ingestion/websocket.gateway";
import { ValidationService } from "../src/ingestion/validation.service";
import { RoutingService } from "../src/processing/routing.service";
import { BatchController } from "../src/ingestion/batch.controller";
import { MetricsService } from "../src/monitoring/metrics.service";
import { AlertsService } from "../src/monitoring/alerts.service";
import { RedisClient, ClickHouseClient } from "@libs/database";
import { Logger } from "@libs/monitoring";
import { createElysiaServer, t } from "@libs/elysia-server";
import { RegistryService } from "../src/schema/registry.service";
import { ValidatorService as SchemaValidatorService } from "../src/schema/validator.service";
import { MigrationService } from "../src/schema/migration.service";
import { DeadLetterHandler } from "../src/deadletter/handler.service";
import { RetryService } from "../src/deadletter/retry.service";
import { ServiceContainer } from "./container";

// Import shared middleware
import { eventPipelineMiddleware } from "../src/middleware/setup";
  ...
  (app: any) => {
    return (
      app
        ...
        .post("/alerts", async ({ body, user }: any) => {
        ...
        .post("/schemas", async ({ body, user }: any) => {
          if (!user?.permissions?.includes("admin")) {
        ...
        .put("/schemas/:version", async ({ params, body, user }: any) => {
          if (!user?.permissions?.includes("admin")) {
            return { error: "Admin permission required for schema management" };
          }
        ...
        .post(
          ...
          async ({ params, body, user }: any) => {
            if (!user?.permissions?.includes("admin")) {
              return {
                error: "Admin permission required for schema validation",
              };
            }
        ...
        .get("/deadletter", async ({ user }: any) => {
          if (!user?.permissions?.includes("admin")) {
            return {
              error: "Admin permission required for dead letter access",
            };
          }
        ...
        .post("/deadletter/retry", async ({ body, user }: any) => {
          if (!user?.permissions?.includes("admin")) {
